...
const AvatarPicker = ({ selected, onSelect }) => {
  ...
  return (
    <div className="avatar-picker">
      ...
      <div className="grid grid-cols-4 sm:grid-cols-6 xl:grid-cols-6 gap-2 lg:gap-3 justify-items-center">
        {AVATARS.map((avatar) => (
          <button
            key={avatar.id}
            type="button"
            onClick={() => onSelect(avatar)}
            className={`avatar-option w-10 h-10 sm:w-12 sm:h-12 xl:w-14 xl:h-14 rounded-full bg-linear-to-br ${avatar.bg} flex items-center pt-1 justify-center text-xl sm:text-2xl xl:text-3xl transition-all duration-200 border-0 ${
